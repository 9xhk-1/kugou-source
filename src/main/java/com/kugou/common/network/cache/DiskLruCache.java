...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    private final Runnable cleanupRunnable = new Runnable() { // from class: com.kugou.common.network.cache.DiskLruCache.1
    ...
    public final class Editor {
        ...
        public Sink newSink(int i2) {
            synchronized (DiskLruCache.this) {
                ...
                try {
                    return new FaultHidingSink(DiskLruCache.this.fileSystem.sink(entry.dirtyFiles[i2])) { // from class: com.kugou.common.network.cache.DiskLruCache.Editor.1
                        @Override // com.kugou.common.network.cache.FaultHidingSink
            ...
            while (true) {
                try {
                    ...
                    while (true) {
                        ...
                        KGNetworkUtil.closeQuietly(sourceArr[i2]);
                        i2++;
                    }
                }
            }
    ...
    public final class Snapshot implements Closeable {
        ...
        public void close() {
            for (Source source : this.sources) {
                KGNetworkUtil.closeQuietly(source);
            }
        }
    ...
    public static DiskLruCache create(FileSystem fileSystem, File file, int i2, int i3, long j) {
        ...
        if (i3 > 0) {
            return new DiskLruCache(fileSystem, file, i2, i3, j, new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue(), KGNetworkUtil.threadFactory("kgHttpDiskCache", true)));
        }
        throw new IllegalArgumentException("valueCount <= 0");
    }

    private BufferedSink newJournalWriter() throws FileNotFoundException {
        return Okio.buffer(new FaultHidingSink(this.fileSystem.appendingSink(this.journalFile)) { // from class: com.kugou.common.network.cache.DiskLruCache.2
            public static final /* synthetic */ boolean $assertionsDisabled = false;
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    KGNetworkUtil.closeQuietly(bufferedSourceBuffer);
                    return;
                }
            }
        } catch (Throwable th) {
    ...
    public synchronized void initialize() throws IOException {
        ...
        if (this.fileSystem.exists(this.journalFile)) {
            try {
                ...
                NetLog.d("", "DiskLruCache " + this.directory + " is corrupt: " + e2.getMessage() + ", removing " + e2);
                try {
                    delete();
                    this.closed = false;
                    rebuildJournal();
    ...
    public synchronized Iterator<Snapshot> snapshots() throws IOException {
        initialize();
        return new Iterator<Snapshot>() { // from class: com.kugou.common.network.cache.DiskLruCache.3
