...
public final class Downsampler {
    ...
    private static int adjustTargetDensityForError(double d2) {
        ...
        Double.isNaN(d3);
        int iRound = round(d3 * d2);
        double d4 = iRound / densityMultiplier;
        Double.isNaN(d4);
        double d5 = iRound;
        Double.isNaN(d5);
    ...
    private Bitmap decodeFromWrappedStreams(ImageReader imageReader, BitmapFactory.Options options, DownsampleStrategy downsampleStrategy, DecodeFormat decodeFormat, PreferredColorSpace preferredColorSpace, boolean z, int i2, int i3, boolean z2, DecodeCallbacks decodeCallbacks) throws IOException {
        ...
        calculateConfig(imageReader, decodeFormat, z4, zIsExifOrientationRequired, options, i5, i9);
        int i10 = Build.VERSION.SDK_INT;
        boolean z5 = i10 >= 19;
        if (options.inSampleSize == 1 || z5) {
            ...
            if (downsampler.shouldUsePool(imageType)) {
                if (i7 < 0 || i8 < 0 || !z2 || !z5) {
                    ...
                    if (Log.isLoggable(TAG, 2)) {
                        Log.v(TAG, "Calculated target [" + iRound + "x" + iRound2 + "] for source [" + i7 + "x" + i8 + "], sampleSize: " + i11 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f4);
                    }
                } else {
                    iRound = i5;
                    iRound2 = i9;
        ...
        if (Log.isLoggable(TAG, 2)) {
            i6 = imageOrientation;
            logDecode(i7, i8, str, options, bitmapDecodeStream, i2, i3, logTime);
        } else {
            i6 = imageOrientation;
        }
    ...
    */
    ...
    private static void logDecode(int i2, int i3, String str, BitmapFactory.Options options, Bitmap bitmap, int i4, int i5, long j) {
