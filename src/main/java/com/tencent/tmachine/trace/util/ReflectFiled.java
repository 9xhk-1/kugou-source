...
public class ReflectFiled<Type> {
    ...
    public synchronized Type getWithoutThrow(Object obj) {
        ...
        try {
            try {
                ...
                TMachineLog.i(TAG, "getWithoutThrow, exception occur :%s", e2);
                type = null;
                return type;
            } catch (IllegalArgumentException e3) {
                TMachineLog.i(TAG, "getWithoutThrow, exception occur :%s", e3);
                type = null;
                return type;
            }
        } catch (NoSuchFieldException e4) {
            TMachineLog.i(TAG, "getWithoutThrow, exception occur :%s", e4);
            type = null;
            return type;
        }
        return type;
    ...
    public synchronized boolean setWithoutThrow(Object obj, Type type) {
        ...
        try {
            try {
                ...
                TMachineLog.i(TAG, "setWithoutThrow, exception occur :%s", e2);
            } catch (IllegalArgumentException e3) {
                TMachineLog.i(TAG, "setWithoutThrow, exception occur :%s", e3);
            }
        } catch (NoSuchFieldException e4) {
            TMachineLog.i(TAG, "setWithoutThrow, exception occur :%s", e4);
        }
        return z;
    }
    ...
    public synchronized Type get(boolean z) throws IllegalAccessException, NoSuchFieldException, IllegalArgumentException {
        ...
        TMachineLog.w(TAG, String.format("Field %s is no exists.", this.mFieldName), new Object[0]);
        return null;
    }
