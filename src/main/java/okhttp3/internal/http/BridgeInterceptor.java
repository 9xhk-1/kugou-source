...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request.header(HTTP.TARGET_HOST) == null) {
            builderNewBuilder.header(HTTP.TARGET_HOST, Util.hostHeader(request.url(), false));
        }
        ...
        if (request.header(HTTP.USER_AGENT) == null) {
            builderNewBuilder.header(HTTP.USER_AGENT, Version.userAgent());
        }
        ...
        if (z && "gzip".equalsIgnoreCase(responseProceed.header(HTTP.CONTENT_ENCODING)) && HttpHeaders.hasBody(responseProceed)) {
            GzipSource gzipSource = new GzipSource(responseProceed.body().source());
            builderRequest.headers(responseProceed.headers().newBuilder().removeAll(HTTP.CONTENT_ENCODING).removeAll("Content-Length").build());
