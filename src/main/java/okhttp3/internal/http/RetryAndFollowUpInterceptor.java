...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    private Address createAddress(Request request) {
        ...
        Proxy requestProxy = request.getRequestProxy() != null ? request.getRequestProxy() : this.client.proxy();
        Dns dnsPrivateDns = request.privateDns();
        return new Address(httpUrlUrl.host(), httpUrlUrl.port(), dnsPrivateDns == null ? this.client.dns() : dnsPrivateDns, this.client.socketFactory(), sSLSocketFactory, hostnameVerifier, certificatePinner, this.client.proxyAuthenticator(), requestProxy, this.client.protocols(), this.client.connectionSpecs(), this.client.proxySelector());
    }
    ...
    private Request followUpRequest(Response response, Route route) throws IOException {
        ...
        if (iCode == 307 || iCode == 308) {
            if (!strMethod.equals("GET") && !strMethod.equals(HttpHead.METHOD_NAME)) {
                return null;
            }
        } else {
            if (iCode == 401) {
        ...
        if (!this.client.followRedirects() || (strHeader = response.header(HttpUrlFetcher.REDIRECT_HEADER_FIELD)) == null || (httpUrlResolve = response.request().url().resolve(strHeader)) == null) {
            return null;
        }
        ...
        if (!sameConnection(response, httpUrlResolve)) {
            builderNewBuilder.removeHeader(AUTH.WWW_AUTH_RESP);
        }
    ...
    private int retryAfter(Response response, int i2) {
        ...
        return strHeader == null ? i2 : strHeader.matches("\\d+") ? Integer.valueOf(strHeader).intValue() : ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
    }
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        StreamAllocation streamAllocation = new StreamAllocation(this.client.connectionPool(), createAddress(request), call, eventListener, this.callStackTrace);
            ...
            try {
                ...
                try {
                    ...
                    if (!sameConnection(responseProceed, requestFollowUpRequest.url())) {
                        streamAllocation.release();
                        streamAllocation = new StreamAllocation(this.client.connectionPool(), createAddress(requestFollowUpRequest), call, eventListener, this.callStackTrace);
                        this.streamAllocation = streamAllocation;
