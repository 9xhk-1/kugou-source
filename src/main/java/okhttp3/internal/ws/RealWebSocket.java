...
public final class RealWebSocket implements WebSocket, WebSocketReader.FrameCallback {
    ...
    public void checkResponse(Response response) throws ProtocolException {
        if (response.code() != 101) {
            throw new ProtocolException("Expected HTTP 101 response but was '" + response.code() + " " + response.message() + LyricManager.STR_REPLACE_RESULT_TAG);
        }
        String strHeader = response.header(HTTP.CONN_DIRECTIVE);
        if (!"Upgrade".equalsIgnoreCase(strHeader)) {
            throw new ProtocolException("Expected 'Connection' header value 'Upgrade' but was '" + strHeader + LyricManager.STR_REPLACE_RESULT_TAG);
        }
        String strHeader2 = response.header("Upgrade");
        if (!"websocket".equalsIgnoreCase(strHeader2)) {
            throw new ProtocolException("Expected 'Upgrade' header value 'websocket' but was '" + strHeader2 + LyricManager.STR_REPLACE_RESULT_TAG);
        }
        ...
        throw new ProtocolException("Expected 'Sec-WebSocket-Accept' header value '" + strBase64 + "' but was '" + strHeader3 + LyricManager.STR_REPLACE_RESULT_TAG);
    }
    ...
    public void connect(OkHttpClient okHttpClient) {
        ...
        final Request requestBuild = this.originalRequest.newBuilder().header("Upgrade", "websocket").header(HTTP.CONN_DIRECTIVE, "Upgrade").header("Sec-WebSocket-Key", this.key).header("Sec-WebSocket-Version", "13").build();
    ...
    public synchronized boolean close(int i2, String str, long j) {
        ...
        if (!this.failed && !this.enqueuedClose) {
            ...
            this.messageAndCloseQueue.add(new Close(i2, byteStringEncodeUtf8, j));
            runWriter();
            return true;
        }
        return false;
